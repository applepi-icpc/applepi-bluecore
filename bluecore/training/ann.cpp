...
const int CNT_REAL_DATA = 2067;
const int CNT_TRAINDATA = CNT_SAMPLE * N_OUTPUT * (MAX_OFFSET * 2 + 1) * (MAX_OFFSET * 2 + 1) + CNT_REAL_DATA;
...
int main () {
	...
	for (int i = 0; i < CNT_SAMPLE; i++) {
		for (int j = 0; j < N_OUTPUT; j++) if (decode(j) != '0' && decode(j) != 'o' && decode(j) != 'O' && decode(j) != 'l' && decode(j) != 'I' && decode(j) != 'Q' && decode(j) != '1') {
			char tfile[256];
	...
	// Read real data
	for (int i = 0; i < CNT_REAL_DATA; i++) {
		char tfile[256];
		sprintf(tfile, "realdata/data%d.dat", i);
		FILE *fp = fopen(tfile, "r");
		for (int x = 0; x < N_INPUT; x++) {
			fscanf(fp, "%lf", &train_in[t_id][x]);
		}
		char t[5]; fscanf(fp, "%s", t);
		int j = code(t[0]);
		for (int k = 0; k < N_OUTPUT; k++) {
			train_out[t_id][k] = (k == j ? 1.0 : 0.0);
		}
